...
main([SockPath]) ->
    ...
    MaybeLSock = gen_tcp:listen(0,[{ifaddr,{local, SockPath}}, local, binary,
				  {packet, 4}, {active, once}]),
...
listen({ok, LSock}, #{sock_path := SockPath} = M) ->
    ...
    Signals =[sighup, sigquit, sigabrt, sigalrm, sigterm, sigusr1, sigusr2,
...
cleanup(#{lsock := LSock, sock_path := SockPath,
