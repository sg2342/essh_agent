...
request_identities1({error, _} = E) ->
    E;
request_identities1({ok, <<?BYTE(?SSH_AGENT_IDENTITIES_ANSWER), ?UINT32(Count), Data/binary>>}) ->
    request_identities2(Count, Data, []);
request_identities1({ok, _}) ->
    {error, unexpected_data}.

request_identities2(0, <<>>, Acc) ->
    {ok, lists:filtermap(fun request_identities3/1, Acc)};
request_identities2(
    N, <<?BINARY(KeyBlob, _KeyBlobLen), ?BINARY(Comment, _CommentLen), Rest/binary>>, Acc
) ->
    request_identities2(N - 1, Rest, [{KeyBlob, Comment} | Acc]);
request_identities2(_, _, _) ->
    {error, unexpected_data}.

request_identities3({K, C}) ->
    try
        {true, {essh_pkt:dec_signature_key(K), C}}
    catch
...
sign_request(Agent, TBS, #{type_info := _} = Cert) ->
    ...
    Req = <<?BYTE(?SSH_AGENTC_SIGN_REQUEST), ?BINARY(CertBlob), ?BINARY(TBS), ?UINT32(Flags)>>,
    sign_request1(req(Agent, Req));
sign_request(Agent, TBS, SignatureKey) ->
    KeyBlob = essh_pkt:enc_signature_key(SignatureKey),
    Flags = ?SSH_AGENT_RSA_SHA2_512 + ?SSH_AGENT_RSA_SHA2_256,
    Req = <<?BYTE(?SSH_AGENTC_SIGN_REQUEST), ?BINARY(KeyBlob), ?BINARY(TBS), ?UINT32(Flags)>>,
    sign_request1(req(Agent, Req)).

sign_request1({error, _} = E) ->
    E;
sign_request1({ok, <<?BYTE(?SSH_AGENT_FAILURE)>>}) ->
    {error, agent_failure};
sign_request1({ok, <<?BYTE(?SSH_AGENT_SIGN_RESPONSE), ?BINARY(Signature, SingatureLen)>>}) ->
    {ok, Signature};
sign_request1({ok, _}) ->
    {error, unexpected_data}.
...
add_smartcard_key(Agent, Id, Pin) ->
    Req = <<?BYTE(?SSH_AGENTC_ADD_SMARTCARD_KEY), ?BINARY(Id), ?BINARY(Pin)>>,
    simple_req(Agent, Req).
...
-spec remove_smartcard_key(essh_agent(), Id :: binary(), Pin :: binary()) ->
    ok | {error, Reason :: term()}.
remove_smartcard_key(Agent, Id, Pin) when
    is_binary(Id), is_binary(Pin)
->
    Req = <<?BYTE(?SSH_AGENTC_REMOVE_SMARTCARD_KEY), ?BINARY(Id), ?BINARY(Pin)>>,
    simple_req(Agent, Req).
...
-spec add_smartcard_key_constrained(
    essh_agent(),
    Id :: binary(),
    Pin :: binary(),
    [essh_constraint()]
